...
class expansionPenaltyFunction(Function):
    ...
    def forward(ctx, xyz, primitive_size, alpha):
        ...
        dist = torch.zeros(batchsize, n, device="cuda").contiguous()            # torch.Size([2, 16384])个0
        assignment = torch.zeros(batchsize, n, device="cuda", dtype=torch.int32).contiguous() - 1       # torch.Size([2, 16384])个-1
        neighbor = torch.zeros(batchsize, n * 512, device="cuda", dtype=torch.int32).contiguous()       # torch.Size([2, 8388608])个0
        cost = torch.zeros(batchsize, n * 512, device="cuda").contiguous()                              # torch.Size([2, 8388608])个0
        mean_mst_length = torch.zeros(batchsize, device="cuda").contiguous()                            # 2个0
        expansion_penalty.forward(
            xyz, primitive_size, assignment, dist, alpha, neighbor, cost, mean_mst_length
        )
